...
prepare-image:  ## Prepare the image for release.
	...
	REPOSITORY=manloralm/mca-tfm-poc
	LATEST_TAG=$(shell curl -s "https://hub.docker.com/v2/repositories/$(REPOSITORY)/tags/" | jq -r '.results[].name' | sort -V | tail -n1)
	LATEST_RC=$(shell echo "$(LATEST_TAG)" | awk -F-rc '{print $NF}')
	NEXT_RC=$(expr $(LATEST_RC) + 1)
